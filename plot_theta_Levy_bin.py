...
for i,n in enumerate(range(10,11)):
    ...
    for iw, mu in enumerate(mu_vec):
        ...
        if mu == 0.1 or mu == 0.2:
            ax.plot(w_plot[:-1],theta_smooth,"-",linewidth=0.8,color=cmap(norm(mu)))
            ...
            ax.plot(w_plot[:ind_min_theta],theta_smooth[:ind_min_theta],"-",linewidth=0.8,color=cmap(norm(mu)))
